...
\begin{enumerate}
\item $\quad \alpha, \beta \in \znz,\qquad(\alpha\beta)^{-1} = \beta^{-1} \alpha^{-1}$
\item $\quad \alpha^{-1} \in \znz,\qquad (\alpha^{-1})^{-1} = \alpha $
\end{enumerate}
}\begin{proof} Vale:
\begin{enumerate}
...
\end{align*}
Sia ora $\{\beta_1, \beta_2, \ldots , \beta_k\} \in \znz$ con $k = \phi(n)$. Applicando $ L_{\alpha}$ si ottiene
